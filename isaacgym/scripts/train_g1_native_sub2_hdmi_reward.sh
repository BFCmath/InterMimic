...
DEFAULT_RUN_NAME="g1_native_sub2_hdmi_reward"
...
if [ "$HEADLESS_SET" -eq 0 ] && [ -z "${DISPLAY:-}" ]; then
    ...
    echo "[train_g1_native_sub2_hdmi_reward] DISPLAY is not set; forcing --headless to avoid Isaac Gym viewer segfaults"
fi
...
TMP_CFG_ENV="$(mktemp "${TMPDIR:-/tmp}/omomo_g1_native.sub2.hdmi.XXXXXX.yaml")"
...
TMP_CFG_TRAIN="$(mktemp "${TMPDIR:-/tmp}/omomo_g1_native.sub2.hdmi.${RUN_NAME}.XXXXXX.yaml")"
...
echo "[train_g1_native_sub2_hdmi_reward] run_name=$RUN_NAME"
echo "[train_g1_native_sub2_hdmi_reward] dataSub=['sub2']"
echo "[train_g1_native_sub2_hdmi_reward] motion_file=/tmp/intermimic_sub2_native"
echo "[train_g1_native_sub2_hdmi_reward] fakeReward=0"
echo "[train_g1_native_sub2_hdmi_reward] hdmiReward=1"
echo "[train_g1_native_sub2_hdmi_reward] checkpoint_dir=$DEFAULT_OUTPUT_PATH$RUN_NAME/nn"
if [ "$CHECKPOINT_SET" -eq 0 ] && is_valid_checkpoint "$RESUME_PATH"; then
    AUTO_RESUME_ARGS="--checkpoint $RESUME_PATH --resume 1"
    echo "[train_g1_native_sub2_hdmi_reward] auto_resume=$RESUME_PATH"
elif [ "$CHECKPOINT_SET" -eq 0 ] && [ -f "$RESUME_PATH" ]; then
    echo "[train_g1_native_sub2_hdmi_reward] skip_corrupt_resume=$RESUME_PATH"
fi
